...
const [employeeStats, attendanceStats, leaveStats] = await Promise.all([
  ...
  // Lazy loading for recent activity (example)
  ...
  const fetchActivity = async (reset = false) => {
    ...
    // Example: /api/activity?page=activityPage&pageSize=activityPageSize
    const fakeData = [
      { text: 'John Doe marked attendance', time: '2 minutes ago' },
      { text: 'Sarah Smith submitted leave request', time: '1 hour ago' },
      { text: 'Performance review completed for Mike Johnson', time: '3 hours ago' },
      { text: 'New employee onboarding started', time: '1 day ago' },
      // ...simulate more activity objects here...
    ];
    if (reset) {
      setActivity(fakeData.slice(0, activityPageSize));
      setActivityPage(2);
      setActivityHasMore(fakeData.length > activityPageSize);
    } else {
      setActivity((prev) => [...prev, ...fakeData.slice((activityPage-1)*activityPageSize, activityPage*activityPageSize)]);
      setActivityPage((prev) => prev + 1);
      setActivityHasMore(fakeData.length > activityPage * activityPageSize);
    }
  };
  ...
  const handleActivityScroll = (e) => {
    ...
    if (scrollHeight - scrollTop === clientHeight && activityHasMore) {
      fetchActivity();
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="card">
          ...
          <div className="card-content px-8 pb-8 overflow-auto" style={{ maxHeight: 300 }} onScroll={handleActivityScroll}>
